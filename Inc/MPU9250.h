...
//===================================================================================================================
    ...
    void writeByte(uint8_t subAddress, uint8_t data)
    {
        ...
        data_write[1] = data;
        HAL_SPI_Transmit(&mpu_spi, data_write, 2, 1000);
        wait(0.00001);
        mpu_spi_deselect();
    }
    ...
    void writeByteAK(uint8_t subAddress, uint8_t data)
    {
        ...
        //WriteReg(MPUREG_,AK8963_I2C_ADDR|READ_FLAG); //Set the I2C slave addres of AK8963 and set for read.
        writeByte(I2C_SLV0_REG, subAddress);
        //WriteReg(MPUREG_I2C_SLV0_REG, AK8963_HXL); //I2C slave 0 register address from where to begin data transfer

        writeByte(I2C_SLV0_CTRL, 0x86);    //Enable I2C and set bytes
*/
    ...
    char readByte(uint8_t subAddress)
    {
        ...
        data_write[0] = subAddress | 0x80;
        HAL_SPI_Transmit(&mpu_spi, data_write, 1, 1000); // no stop
        HAL_SPI_Receive(&mpu_spi, data, 1, 1000); 
        mpu_spi_deselect();
        return data[0]; 
    }
    ...
    void readBytes(uint8_t subAddress, uint8_t count, uint8_t * dest)
    {
        ...
        data_write[0] = subAddress | 0x80;
        HAL_SPI_Transmit(&mpu_spi, data_write, 1, 1000); // no stop
        HAL_SPI_Receive(&mpu_spi, data, count, 1000); 
...
void readAccelData(int16_t * destination)
{
    ...
    //readBytes(ACCEL_XOUT_H, 2, &rawData[0]);  // Read the six raw data registers into data array
    //readBytes(ACCEL_YOUT_H, 2, &rawData[2]);  // Read the six raw data registers into data array
    //readBytes(ACCEL_ZOUT_H, 2, &rawData[4]);  // Read the six raw data registers into data array
...
{  
  ...
  // Set clock source to be PLL with x-axis gyroscope reference, bits 2:0 = 001
    writeByte(PWR_MGMT_1, 0x01); // Auto select clock source 
    writeByte(PWR_MGMT_2, 0x00); // Enable acc and gyro
    wait(0.2);
  ...
  // Configure device for bias calculation
    ...
    writeByte(USER_CTRL, 0x10);    // Disable FIFO and I2C master modes
  ...
  // Configure FIFO to capture accelerometer and gyro data for bias calculation
    ...
    writeByte(FIFO_EN, 0x78);     // Enable gyro and accelerometer sensors for FIFO (max size 512 bytes in MPU-9250)
...
void MPU9250SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
    ...
    writeByte(GYRO_CONFIG, 0<<FS); // Set full scale range for the gyro to 250 dps
    writeByte(ACCEL_CONFIG2, 0x02); // Set accelerometer rate to 1 kHz and bandwidth to 92 Hz
    writeByte(ACCEL_CONFIG, 0<<FS); // Set full scale range for the accelerometer to 2 g
